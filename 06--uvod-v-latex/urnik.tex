\documentclass[4Apaper, 10pt]{article}
...
\[
 \texttt{velikostTabele} = \texttt{številoUrNaDan} \cdot \texttt{številoDelovnihDnih}
\]
...
\textsl{Operacija} je vsaka sprememba oziroma poseg v urnik med izvajanjem optimizacije.
Na primeru urnika si oglejmo, katere so smiselne operacije:

% Vsaka od vrstic v okolju `itemize` je en element naštevanja.
\begin{itemize}
   \item \textsl{Vstavljanje} srečanja v urnik.
   \item \textsl{Brisanje} srečanja iz urnika.
   \item \textsl{Zamenjava} dveh srečanj v urnik.
\end{itemize}
\noindent
% Spodnja vrstica ne začne novega odstavka, zato ne bi smela biti zamaknjena.
% To popravite z ukazom `\noindent` ali pa tako, da odstranite prazno vrstico.
% (Nekoliko lepše izgleda, če v prazno vrstico vstavite % za komentar.)
\textsl{Okolica} operacije je množica vseh možnih sprememb, ki jih lahko s to operacijo
...
\textsl{Sistem} okolic je množica vseh možnih sprememb, ki jih lahko z vsemi operacijami iz
...
seznam tabujev. V naslednjem koraku izvedemo spremembo $s_1$, ki se prav tako shrani v
...
\item izvajamo, dokler ne dosežemo enega izmed ustavitvenih pogojev.
